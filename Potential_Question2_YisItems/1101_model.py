...
# In[]: 宣告函數
def Data_Aggregate_Clip_Scal(dataset_name_list,  Dataset, Clipper, Scaler, delete_fifth=False):
    ...
    if delete_fifth == True :
        data_temp = np.delete(data_temp , 5, 1) # delete fifth column which is original Na Value
    #Clipping
    if Clipper == None :     #若未給定Clipper，則重新fit一個Clipper
        ...
        for i in range(data_temp.shape[1]):
            ...
            if len(nonzero)>0 : #每一欄至少要有一個非0的值,才clip，因為會出現某一總商品皆未被購買的情況
                clip_temp = np.percentile(nonzero, 75, axis=0) + (3*(np.percentile(nonzero, 75, axis=0) - np.percentile(nonzero, 25, axis=0)))
...
def generate_performance_table_RP(y_true, y_pred):
    ...
    for i in range( 0,(train_y3_True.shape)[0]): #每一列(會員)，用實際的Item vs 預測的Item，來計算一個confusion matrix
        cm = confusion_matrix(y_true = y_true[i,:],
                         y_pred = y_pred[i,:] )
        if cm.shape == (2, 2): #防止某些會員Item購買量全為0
            cm_temp.append([(cm[1,0] +  cm[1,1]) , cm[1,1], (cm[0,1] +  cm[1,1]) , cm[1,1]])
        else:
            cm_temp.append([0, 0, 0, 0])#防止某些會員Item購買量全為0，若發生此狀況，則該會員回傳0
        if i % 10000 == 0 :
            print(i)
...
def Scatterplot_RP( performance_table, NUll_performance_table, lower = 0, upper = 500) : 
    ...
    print('Recall')
    xTrue = performance_table['True1']
    yPredict = performance_table['Predict1_of_true1']
    xTrue_NUll = NUll_performance_table['True1']
    yPredict_NUll = NUll_performance_table['Predict1_of_true1']
    xlabel = 'True1'
    ylabel = 'Predict1_of_true1'

    plt.scatter(x=xTrue_NUll, y=yPredict_NUll, marker =  ".", color='green', s=1, alpha =0.1) 
    plt.scatter(x=xTrue, y=yPredict, marker =  ".", color='blue', s=1, alpha =0.1) 
...
def SummarizeHistory(modelcallbacks) :
    ...
    fbeta_score
    plt.figure()
    plt.plot(modelcallbacks.history['fbeta_score'])  
    plt.plot(modelcallbacks.history['val_fbeta_score'])  
    plt.title('model loss')
    plt.ylabel('loss')
    plt.xlabel('epoch')
    plt.legend(['fbeta_score', 'val_fbeta_score'], loc='upper left')
    plt.show()
...
def FinalCompare_Mb_Recall(Null_temp, M1_temp, M2_temp, M3_temp):  #依人的recall比較模型
    #看哪一個模型好
    df_temp = pd.DataFrame(data={'M0':Null_temp,'M1': M1_temp,'M2':M2_temp,'M3':M3_temp})
    df_temp.fillna(0, inplace = True)
    df_temp['BestModel'] = df_temp.idxmax(axis=1)
    df_temp.BestModel.fillna('fare', inplace = True) #Na當作平手
    for i in range(0, df_temp.shape[0]): 
        if (df_temp.iloc[i,0] == df_temp.iloc[i,1]) | (df_temp.iloc[i,1] == df_temp.iloc[i,2]) | (df_temp.iloc[i,2] == df_temp.iloc[i,3]) :
            if(df_temp.iloc[i,0] == df_temp.iloc[i,1]) & (df_temp.iloc[i,1] == df_temp.iloc[i,2]) & (df_temp.iloc[i,2] == df_temp.iloc[i,3]) :
                df_temp.loc[i,'BestModel']='total-fare'
            else: 
                df_temp.loc[i,'BestModel']='partial-fare'
    #長條圖
    from collections import Counter
    plt.figure()
    a=Counter(df_temp.BestModel)
    #plt.bar(a.keys(),a.values())
    df = pd.DataFrame.from_dict(a, orient='index').reset_index().sort_values(by='index')
    plt.bar(df.iloc[:,0], df.iloc[:,1])
    
    #看哪一個模型好
    df_temp2 = pd.DataFrame(data={'0':Null_temp,'1':M1_temp,'2':M2_temp,'3':M3_temp})
    df_temp2.fillna(0, inplace = True)
    
    BestModel=[]
    for i in range(0, df_temp2.shape[0]):
        if(df_temp.iloc[i,0] == df_temp.iloc[i,1]) & (df_temp.iloc[i,1] == df_temp.iloc[i,2]) & (df_temp.iloc[i,2] == df_temp.iloc[i,3]) :
            BestModel.append('total-fare')
        else :
            if (df_temp2.iloc[i,0] >= df_temp2.iloc[i,1]) & (df_temp2.iloc[i,0] >= df_temp2.iloc[i,2]) & (df_temp2.iloc[i,0] >= df_temp2.iloc[i,3]) :
                BestModel.append('M0')
            if (df_temp2.iloc[i,1] >= df_temp2.iloc[i,0]) & (df_temp2.iloc[i,1] >= df_temp2.iloc[i,2]) & (df_temp2.iloc[i,1] >= df_temp2.iloc[i,3]) :
                BestModel.append('M1')
            if (df_temp2.iloc[i,2] >= df_temp2.iloc[i,1]) & (df_temp2.iloc[i,2] >= df_temp2.iloc[i,0]) & (df_temp2.iloc[i,2] >= df_temp2.iloc[i,3]) :
                BestModel.append('M2')
            if (df_temp2.iloc[i,3] >= df_temp2.iloc[i,1]) & (df_temp2.iloc[i,3] >= df_temp2.iloc[i,2]) & (df_temp2.iloc[i,3] >= df_temp2.iloc[i,0]) :
                BestModel.append('M3')
    #長條圖
    plt.figure()
    a=Counter(BestModel)
    #plt.bar(a.keys(),a.values())
    df1 = pd.DataFrame.from_dict(a, orient='index').reset_index().sort_values(by='index')
    plt.bar(df1.iloc[:,0], df1.iloc[:,1])
    print(df1)
# In[]: Myloss and metric
...
# Myloss
def fbeta_bce_loss(y_true, y_pred, beta = 0.1):
...
train_x3, Clipper_train_x3, Scaler_train_x3 = Data_Aggregate_Clip_Scal(["M7WD","M8WD","M9WD","M7WE","M8WE","M9WE"], 
                                                     Dataset = Item3, Clipper=None, Scaler=None, delete_fifth=True)
...
valid_x3, _, _ = Data_Aggregate_Clip_Scal(["M10WD","M8WD","M9WD","M10WE","M8WE","M9WE"], 
                                                     Dataset = Item3, Clipper=Clipper_train_x3, Scaler=Scaler_train_x3, delete_fifth=True) # _代表不需要用到

valid_y3_True, _, Scaler_valid_y3 = Data_Aggregate_Clip_Scal(["M11WD","M11WE"], 
                                                     Dataset = Item3, Clipper=Clipper_train_y3_True, Scaler=Scaler_train_y3,  delete_fifth=True)
...
valid_x3_cul, _, _ = Data_Aggregate_Clip_Scal(dataset_name_list = list_temp, 
                                                     Dataset = Item3,  Clipper=Clipper_train_x3_cul, Scaler=Scaler_train_x3_cul, delete_fifth=True) 
# In[]: 初始跑一次就好   
# 生成 True Y
train_y3_True_01 = np.where(train_y3_True> 0, 1, 0) #y_true
valid_y3_True_01 = np.where(valid_y3_True> 0, 1, 0) #y_true
train_x3_01 = np.where(train_x3> 0, 1, 0) 
valid_x3_01 = np.where(valid_x3> 0, 1, 0) 
train_x3_cul_01 = np.where(train_x3_cul> 0, 1, 0) 
valid_x3_cul_01 = np.where(valid_x3_cul> 0, 1, 0) 
# 生成 Null Modle Y 
...
# In[]:M1---------------------------------  
K.clear_session()
A1 = Input(shape = (2001,),name='Input')
...
P = Dropout(0.4)(P)
y = Dense(2001, activation='sigmoid',name='Output')(P)
adam = optimizers.Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
adagrad = optimizers.Adagrad(lr=0.0002, epsilon=None, decay=0.0)
#History_temp = []
for i in range(0,1) :
    model1 = Model(inputs=[A1], outputs=[y])
    #"binary_crossentropy", fbeta_bce_loss
    model1.compile(loss = "binary_crossentropy", optimizer =  adam, metrics=[fbeta_score, my_mean_squared_error, my_mean_absolute_error]) #mean_absolute_error
    model1.summary()
    modelcallbacks1_1 = model1.fit(x = train_x3, y = train_x3_01,
                    epochs=10, batch_size=64, shuffle=True,
                    validation_data=(train_x3, train_y3_True_01,))
    SummarizeHistory(modelcallbacks1_1)
    
    modelcallbacks1_2 = model1.fit(x = train_x3, y = train_y3_True_01,
                   epochs=10, batch_size=64, shuffle=True,
                   validation_data=(valid_x3, valid_y3_True_01,))
    SummarizeHistory(modelcallbacks1_2)
    #Save & Load weights
    Now = datetime.datetime.now()
    model1.save(  'M1_model_weights_'+ str( Now ).replace(" ", "-").replace(":", "_").replace(".", "_")+'.h5'  )
    #model1=load_model('save_M1_model_weights_2018-10-16-08_37_34_428512.h5',custom_objects={'fbeta_bce_loss':fbeta_bce_loss,'my_mean_squared_error': my_mean_squared_error, 'my_mean_absolute_error':my_mean_absolute_error})
    

# In[]: 2.M1生出預測值
train_y3_hat_M1 = model1.predict(train_x3) 
train_y3_hat_inverse_transform_M1 = Scaler_train_y3.inverse_transform(train_y3_hat_M1)
train_y3_hat_01_M1 = np.where(train_y3_hat_inverse_transform_M1> 0.05, 1, 0)
#np.savetxt("train_y3_hat.csv", train_y3_hat, delimiter=",") #輸出CSV

valid_y3_hat_M1 = model1.predict(valid_x3) 
valid_y3_hat_inverse_transform_M1 = Scaler_train_y3.inverse_transform(valid_y3_hat_M1)
valid_y3_hat_01_M1 = np.where(valid_y3_hat_inverse_transform_M1> 0.05, 1, 0)
...
M1_performance_table = generate_performance_table_RP(y_true=train_y3_True_01, y_pred=train_y3_hat_01_M1)
M1_performance_table_valid = generate_performance_table_RP(y_true=valid_y3_True_01, y_pred=valid_y3_hat_01_M1)
...
M1_performance_table_CompareWithNull = Generate_Performance_table_CwN(train_y3_True_01, train_y3_hat_NUll, train_y3_hat_01_M1)
M1_performance_table_valid_CompareWithNull = Generate_Performance_table_CwN(valid_y3_True_01, valid_y3_hat_NUll, valid_y3_hat_01_M1)
...
A3 = Input(shape = (2001,),name='Input_3')
...
for i in range(1,5) :
    ...
    model2.compile(loss = "binary_crossentropy", optimizer =  adam, metrics=[fbeta_score, my_mean_squared_error, my_mean_absolute_error]) #mean_absolute_error
    model2.summary()
    
    modelcallbacks2_1 = model2.fit(x = [train_x1, train_x2, train_x3], y = train_x3_01,
                       epochs=10, batch_size=64, shuffle=True,
                       validation_data=([train_x1, train_x2, train_x3], train_y3_True_01,),
                       callbacks=[EarlyStopping(monitor='loss', patience=1, verbose=0, mode='auto')])
    SummarizeHistory(modelcallbacks2_1)
    
    modelcallbacks2_2 = model2.fit(x = [train_x1, train_x2, train_x3], y = train_y3_True_01,
                      epochs=10, batch_size=64, shuffle=True,
                      validation_data=([valid_x1, valid_x2, valid_x3], valid_y3_True_01,),
                      callbacks=[EarlyStopping(monitor='loss', patience=1, verbose=0, mode='auto')])
    
    SummarizeHistory(modelcallbacks2_2)
    
    Now = datetime.datetime.now()
    print(Now, '-------------------------------------')
   # History_temp.append([Now, model2, modelcallbacks2_1,modelcallbacks2_2 ])
    
    model2.save(  'M2_model_weights_'+ str(Now ).replace(" ", "-").replace(":", "_").replace(".", "_")+'.h5'  )
#model2=load_model('save_M2_model_weights_2018-10-17-11_13_50_617058.h5',custom_objects={'fbeta_score':fbeta_score, 'my_mean_squared_error': my_mean_squared_error, 'my_mean_absolute_error':my_mean_absolute_error})
# In[]: 2.M2生出預測值
train_y3_hat_M2 = model2.predict([train_x1,train_x2,train_x3]) #M2
train_y3_hat_inverse_transform_M2 = Scaler_train_y3.inverse_transform(train_y3_hat_M2)
train_y3_hat_01_M2 = np.where(train_y3_hat_inverse_transform_M2> 0.05, 1, 0)

valid_y3_hat_M2 = model2.predict([valid_x1,valid_x2,valid_x3]) #M2
valid_y3_hat_inverse_transform_M2 = Scaler_train_y3.inverse_transform(valid_y3_hat_M2)
valid_y3_hat_01_M2 = np.where(valid_y3_hat_inverse_transform_M2> 0.05, 1, 0)
# In[] 3. M2畫散布圖 Scatterplot_RP + Compare With NullModel
#呼叫函數generate_performance_table_RP，來生成performance_table
M2_performance_table = generate_performance_table_RP(y_true=train_y3_True_01, y_pred=train_y3_hat_01_M2)
M2_performance_table_valid = generate_performance_table_RP(y_true=valid_y3_True_01, y_pred=valid_y3_hat_01_M2)
...
#呼叫函數generate_performance_table_RP，來生成performance_table
M2_performance_table_CompareWithNull = Generate_Performance_table_CwN(train_y3_True_01, train_y3_hat_NUll, train_y3_hat_01_M2)
M2_performance_table_valid_CompareWithNull = Generate_Performance_table_CwN(valid_y3_True_01, valid_y3_hat_NUll, valid_y3_hat_01_M2)
...
A3_cul = Input(shape = (2001,),name='Input_3_cul')
Merged = concatenate([A1, A2, A3])
Merged_cul = concatenate([A1_cul, A2_cul, A3_cul ])
B = Dense(1000)(Merged)
B = LeakyReLU(alpha=1e-1)(B)
B = Dropout(0.4)(B)
B_cul = Dense(1000)(Merged_cul)
B_cul = LeakyReLU(alpha=1e-1)(B_cul)
B_cul = Dropout(0.4)(B_cul)
Merged_B = concatenate([B, B_cul])
C = Dense(1000,)(Merged_B)
C = LeakyReLU(alpha=1e-1)(C)
C = Dropout(0.4)(C)
D = Dense(1000,)(C)
D = LeakyReLU(alpha=1e-1)(D)
...
A2_cul = Input(shape = (424,),name='Input_2_cul')
A3_cul = Input(shape = (2001,),name='Input_3_cul')
Merged = concatenate([A1_cul, A2_cul, A3_cul])
B = Dense(1000, )(Merged)
B = LeakyReLU(alpha=1e-1)(B)
B = Dropout(0.4)(B)
C = Dense(500, )(B)
...
for i in range(1,2):   
    ...
    model3.compile(loss = "binary_crossentropy", optimizer = adam, metrics=[fbeta_score, my_mean_squared_error, my_mean_absolute_error]) #mean_absolute_error
    model3.summary()#train_x1,train_x2,train_x3,train_x1_cul,train_x2_cul,train_x3_cul
    modelcallbacks3_1 = model3.fit(x = [train_x1,train_x2,train_x3,train_x1,train_x2,train_x3], 
                                   y = train_x3_01,
                    epochs=5, batch_size=64, shuffle=True,
                    validation_data=([train_x1,train_x2,train_x3,train_x1,train_x2,train_x3], 
                                     train_y3_True_01,),
                       callbacks=[EarlyStopping(monitor='loss', patience=2, verbose=0, mode='auto')])
    SummarizeHistory(modelcallbacks3_1)
    Now = datetime.datetime.now()
    print(Now, '-------------------------------------')
    model3.save(  'M3_1_model_weights_'+ str(datetime.datetime.now() ).replace(" ", "-").replace(":", "_").replace(".", "_")+'.h5'  )
    
    modelcallbacks3_2 = model3.fit(x = [train_x1,train_x2,train_x3,train_x1,train_x2,train_x3], 
                                   y = train_y3_True_01,
                    epochs=5, batch_size=64, shuffle=True,
                   validation_data=([valid_x1,valid_x2,valid_x3,valid_x1_cul,valid_x2_cul,valid_x3_cul], valid_y3_True_01,),
                       callbacks=[EarlyStopping(monitor='loss', patience=2, verbose=0, mode='auto')])
    SummarizeHistory(modelcallbacks3_2)
    
    Now = datetime.datetime.now()
    print(Now, '-------------------------------------')
    model3.save(  'M3_model_weights_'+ str(datetime.datetime.now() ).replace(" ", "-").replace(":", "_").replace(".", "_")+'.h5'  )
    #model3=load_model('save_M3_model_weights_2018-10-17-17_32_35_161263.h5',custom_objects={'fbeta_score':fbeta_score, 'my_mean_squared_error': my_mean_squared_error, 'my_mean_absolute_error':my_mean_absolute_error})
# In[]: M3  使用M2模型，只使用累積資料
A1 = Input(shape = (99,),name='Input_1')
A2 = Input(shape = (424,),name='Input_2')
A3 = Input(shape = (2001,),name='Input_3')
Merged = concatenate([A1, A2, A3])
...
History_temp = []
for i in range(1,20) :
    model3 = Model(inputs=[A1,A2,A3], outputs=[y])
    model3.compile(loss = "binary_crossentropy", optimizer =  adam, metrics=[fbeta_score, my_mean_squared_error, my_mean_absolute_error]) #mean_absolute_error
    model3.summary()
    
    modelcallbacks2_1 = model3.fit(x = [train_x1_cul, train_x2_cul, train_x3_cul], y = train_x3_01,
                       epochs=10, batch_size=64, shuffle=True,
                       validation_data=([train_x1_cul, train_x2_cul, train_x3_cul], train_y3_True_01,),
                       callbacks=[EarlyStopping(monitor='loss', patience=1, verbose=0, mode='auto')])
    SummarizeHistory(modelcallbacks2_1)
    
    modelcallbacks2_2 = model3.fit(x = [train_x1_cul, train_x2_cul, train_x3_cul], y = train_y3_True_01,
                      epochs=10, batch_size=64, shuffle=True,
                      validation_data=([valid_x1_cul, valid_x2_cul, valid_x3_cul], valid_y3_True_01,),
                      callbacks=[EarlyStopping(monitor='loss', patience=1, verbose=0, mode='auto')])
    ...
    Now = datetime.datetime.now()
    print(Now, '-------------------------------------')
   # History_temp.append([Now, model3, modelcallbacks2_1,modelcallbacks2_2 ])
    
    model3.save(  'M3_model_weights_'+ str(Now ).replace(" ", "-").replace(":", "_").replace(".", "_")+'.h5'  )
#model3=load_model('M3_model_weights_2018-10-30-00_18_10_742855.h5',custom_objects={'fbeta_score':fbeta_score, 'my_mean_squared_error': my_mean_squared_error, 'my_mean_absolute_error':my_mean_absolute_error})
#M3_model_weights_2018-10-29-23_56_47_457410.h5
# In[]: 模型比較-------------------------
        
train_y3_hat_M3 = model3.predict([  train_x1_cul,train_x2_cul,train_x3_cul]) #M3
#[train_x1,train_x2,train_x3,train_x1_cul,train_x2_cul,train_x3_cul]
train_y3_hat_inverse_transform_M3 = Scaler_train_y3.inverse_transform(train_y3_hat_M3)
train_y3_hat_01_M3 = np.where(train_y3_hat_inverse_transform_M3> 0.05, 1, 0)

valid_y3_hat_M3 = model3.predict([valid_x1_cul,valid_x2_cul,valid_x3_cul]) #M3
#valid_x1,valid_x2,valid_x3,valid_x1_cul,valid_x2_cul,valid_x3_cul]
valid_y3_hat_inverse_transform_M3 = Scaler_train_y3.inverse_transform(valid_y3_hat_M3)
valid_y3_hat_01_M3 = np.where(valid_y3_hat_inverse_transform_M3> 0.05, 1, 0)

M3_performance_table = generate_performance_table_RP(y_true=train_y3_True_01, y_pred=train_y3_hat_01_M3)
M3_performance_table_valid = generate_performance_table_RP(y_true=valid_y3_True_01, y_pred=valid_y3_hat_01_M3)

#Bar plot
#Train recall
Null_temp = NUll_performance_table.Predict1_of_true1 
M1_temp = M1_performance_table.Predict1_of_true1 
M2_temp = M2_performance_table.Predict1_of_true1 
M3_temp = M3_performance_table.Predict1_of_true1  
FinalCompare_Mb_Recall(Null_temp, M1_temp, M2_temp, M3_temp)
#Valid recall
Null_temp = NUll_performance_table_valid.Predict1_of_true1 
M1_temp = M1_performance_table_valid.Predict1_of_true1 
M2_temp = M2_performance_table_valid.Predict1_of_true1 
M3_temp = M3_performance_table_valid.Predict1_of_true1  
FinalCompare_Mb_Recall(Null_temp, M1_temp, M2_temp, M3_temp)
#Train precision
Null_temp = NUll_performance_table.true1_of_Predict1 /  NUll_performance_table.Predict1
M1_temp = M1_performance_table.true1_of_Predict1 /  M1_performance_table.Predict1
M2_temp = M2_performance_table.true1_of_Predict1 /  M2_performance_table.Predict1 
M3_temp = M3_performance_table.true1_of_Predict1 /  M3_performance_table.Predict1 
FinalCompare_Mb_Recall(Null_temp, M1_temp, M2_temp, M3_temp)
#Valid precision
Null_temp = NUll_performance_table_valid.true1_of_Predict1 / NUll_performance_table_valid.Predict1
M1_temp = M1_performance_table_valid.true1_of_Predict1 / M1_performance_table_valid.Predict1
M2_temp = M2_performance_table_valid.true1_of_Predict1 / M2_performance_table_valid.Predict1
M3_temp = M3_performance_table_valid.true1_of_Predict1  / M3_performance_table_valid.Predict1
FinalCompare_Mb_Recall(Null_temp, M1_temp, M2_temp, M3_temp)

######################
### Assemble M2 & M3
######################
train_y3_hat_01_M3plusM2 = np.where((train_y3_hat_01_M3 + train_y3_hat_01_M2)> 0, 1, 0)
valid_y3_hat_01_M3plusM2 = np.where((valid_y3_hat_01_M3 + valid_y3_hat_01_M2)> 0, 1, 0)
train_y3_hat_01_M3minusM2 = train_y3_hat_01_M3plusM2 - train_y3_hat_01_M2
valid_y3_hat_01_M3minusM2 = train_y3_hat_01_M3plusM2 - train_y3_hat_01_M2
train_y3_hat_01_M2minusM3 = train_y3_hat_01_M3plusM2 - train_y3_hat_01_M3
valid_y3_hat_01_M2minusM3 = train_y3_hat_01_M3plusM2 - valid_y3_hat_01_M3

M3plusM2_performance_table = generate_performance_table_RP(y_true=train_y3_True_01, y_pred=train_y3_hat_01_M3plusM2)
M3plusM2_performance_table_valid = generate_performance_table_RP(y_true=valid_y3_True_01, y_pred=valid_y3_hat_01_M3plusM2)
M3minusM2_performance_table = generate_performance_table_RP(y_true=train_y3_True_01, y_pred=train_y3_hat_01_M3minusM2)
M3minusM2_performance_table_valid = generate_performance_table_RP(y_true=valid_y3_True_01, y_pred=valid_y3_hat_01_M3minusM2)
M2minusM3_performance_table = generate_performance_table_RP(y_true=train_y3_True_01, y_pred=train_y3_hat_01_M2minusM3)
M2minusM3_performance_table_valid = generate_performance_table_RP(y_true=valid_y3_True_01, y_pred=valid_y3_hat_01_M2minusM3)

#Bar plot for assemble
#Train recall
Null_temp = M2_performance_table.Predict1_of_true1 
M1_temp = M3plusM2_performance_table.Predict1_of_true1 
M2_temp = M3minusM2_performance_table.Predict1_of_true1 
M3_temp = M2minusM3_performance_table.Predict1_of_true1  
FinalCompare_Mb_Recall(Null_temp, M1_temp, M2_temp, M3_temp)
#Valid recall
Null_temp = M2_performance_table_valid.Predict1_of_true1 
M1_temp = M3plusM2_performance_table_valid.Predict1_of_true1 
M2_temp = M3minusM2_performance_table_valid.Predict1_of_true1 
M3_temp = M2minusM3_performance_table_valid.Predict1_of_true1  
FinalCompare_Mb_Recall(Null_temp, M1_temp, M2_temp, M3_temp)

#Train precision
Null_temp = M2_performance_table.true1_of_Predict1 /M2_performance_table.Predict1
M1_temp = M3plusM2_performance_table.true1_of_Predict1 /M3plusM2_performance_table.Predict1
M2_temp = M3minusM2_performance_table.true1_of_Predict1 /M3minusM2_performance_table.Predict1
M3_temp = M2minusM3_performance_table.true1_of_Predict1  /M2minusM3_performance_table.Predict1
FinalCompare_Mb_Recall(Null_temp, M1_temp, M2_temp, M3_temp)
#Valid precision
Null_temp = M2_performance_table_valid.true1_of_Predict1 /M2_performance_table_valid.Predict1
M1_temp = M3plusM2_performance_table_valid.true1_of_Predict1 /M3plusM2_performance_table_valid.Predict1
M2_temp = M3minusM2_performance_table_valid.true1_of_Predict1 /M3minusM2_performance_table_valid.Predict1
M3_temp = M2minusM3_performance_table_valid.true1_of_Predict1  /M2minusM3_performance_table_valid.Predict1
FinalCompare_Mb_Recall(Null_temp, M1_temp, M2_temp, M3_temp)
...
train_y3_hat_01 = np.where(train_y3_hat_inverse_transform> 0.05, 1, 0)
#np.savetxt("train_y3_hat.csv", train_y3_hat, delimiter=",") #輸出CSV
...
valid_y3_hat_01 = np.where(valid_y3_hat_inverse_transform> 0.05, 1, 0)
#np.savetxt("valid_y3_hat.csv", valid_y3_hat, delimiter=",") #輸出CSV
...
#呼叫自定義函數Scatterplot_RP，來畫出散布圖
print(' Train ')
Scatterplot_RP( M3_performance_table, NUll_performance_table, lower = 0, upper = 50 ) 
print(' Valid' )
...
Null_temp = NUll_performance_table_valid.Predict1_of_true1 / NUll_performance_table_valid.True1
M1_temp = M1_performance_table_valid.Predict1_of_true1 / M1_performance_table_valid.True1
M2_temp = M2_performance_table_valid.Predict1_of_true1 / M2_performance_table_valid.True1
M3_temp = M3_performance_table_valid.Predict1_of_true1 / M3_performance_table_valid.True1
...
df_temp['BestModel'] = df_temp.idxmax(axis=1)
df_temp.BestModel.fillna('fare', inplace = True) #Na當作平手
for i in range(0, df_temp.shape[0]): #分數一樣的當作平手
    if (df_temp.iloc[i,0] == df_temp.iloc[i,1]) | (df_temp.iloc[i,1] == df_temp.iloc[i,2]) | (df_temp.iloc[i,2] == df_temp.iloc[i,3]) :
        if(df_temp.iloc[i,0] == df_temp.iloc[i,1]) & (df_temp.iloc[i,1] == df_temp.iloc[i,2]) & (df_temp.iloc[i,2] == df_temp.iloc[i,3]) :
            df_temp.loc[i,'BestModel']='fare'
        else: 
            df_temp.loc[i,'BestModel']='partial-fare'
#長條圖
from collections import Counter
...
df_temp2.fillna(0, inplace = True)
BestModel=[]
for i in range(0, df_temp2.shape[0]): #分數一樣的當作平手
    if (df_temp2.iloc[i,0] >= df_temp2.iloc[i,1]) & (df_temp2.iloc[i,0] >= df_temp2.iloc[i,2]) & (df_temp2.iloc[i,0] >= df_temp2.iloc[i,3]) :
        BestModel.append('M0')
    if (df_temp2.iloc[i,1] >= df_temp2.iloc[i,0]) & (df_temp2.iloc[i,1] >= df_temp2.iloc[i,2]) & (df_temp2.iloc[i,1] >= df_temp2.iloc[i,3]) :
        BestModel.append('M1')
...
"""#計算預測會員與實際會員的相關係數：結果太多NA
def generate_performance_table_corrcoef(y_true, y_pred):
    #y_true=train_y3_inverse_transform
    #y_pred=train_y3_hat_inverse_transform
    corrcoef_temp = [] #先創建一個LIST
    for i in range( 0,(y_true.shape)[0]): 
        corrcoef = np.corrcoef(y_true[i,:],y_pred[i,:] )
        if corrcoef.shape == (2, 2): #防止某些會員Item購買量全為0，若發生此狀況，則該會員的performance不記錄
            corrcoef_temp.append(corrcoef[0,0])
        if i % 1000 == 0 :
            print(i)
    performance_table = pd.DataFrame(data=corrcoef_temp)
    performance_table.rename(columns={0:'corrcoef_temp'},inplace=True)#注意这里0和1都不是字符串
    return performance_table
M3_corrcoef = generate_performance_table_corrcoef(y_true=train_y3_inverse_transform, y_pred=train_y3_hat_inverse_transform)


# cross_entropy
def Generate_Item_cross_entropy(y_true, y_pred):
    #y_true=train_y3_inverse_transform
    #y_pred=train_y3_hat_inverse_transform
    temp = [] #先創建一個LIST
    for i in range( 0,(y_true.shape)[0]): 
        CE = cross_entropy(y_true[i,:],y_pred[i,:] )
        temp.append(CE)
        if i % 1000 == 0 :
            print(i)
    return temp

NUll_Mb_CE = Generate_Item_cross_entropy(y_true=train_y3_01, y_pred=train_y3_hat_NUll_01)
M1_Mb_CE = Generate_Item_cross_entropy(y_true=train_y3_01, y_pred=train_y3_hat_M1_01)
M2_Mb_CE = Generate_Item_cross_entropy(y_true=train_y3_01, y_pred=train_y3_hat_M2_01)
M3_Mb_CE = Generate_Item_cross_entropy(y_true=train_y3_01, y_pred=train_y3_hat_M3_01)

#看哪一個模型好
df_temp = pd.DataFrame(data={'0':NUll_Item_CE,'1':M1_Item_CE,'2':M2_Item_CE,'3':M3_Item_CE})
df_temp.fillna(0, inplace = True)
df_temp['BestModel'] = df_temp.idxmax(axis=1)
df_temp.BestModel.fillna('fare', inplace = True) #Na當作平手
for i in range(0, df_temp.shape[0]): #分數一樣的當作平手
...
NUll_Item_CE = Generate_Item_cross_entropy(y_true=train_y3_01, y_pred=train_y3_hat_NUll_01)
M1_Item_CE = Generate_Item_cross_entropy(y_true=train_y3_01, y_pred=train_y3_hat_M1_01)
M2_Item_CE = Generate_Item_cross_entropy(y_true=train_y3_01, y_pred=train_y3_hat_M2_01)
M3_Item_CE = Generate_Item_cross_entropy(y_true=train_y3_01, y_pred=train_y3_hat_M3_01)
