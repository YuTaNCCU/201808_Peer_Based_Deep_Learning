...
for month in range(1,13):
   for weekday in ["WD","WE"]:
       ...
       Item[str_temp] = pd.read_csv("data/"+str_temp + ".csv").drop(columns=["Unnamed: 0","MbID"]).values  #讀取單品
...
def MyScatterplot( xTrue, yPredict, xTrue_NUll = a, yPredict_NUll = b, limit = 500) : 
    """xTrue = a
    yPredict = b
    xTrue_NUll = an
    yPredict_NUll = bn
...
MyScatterplot( xTrue = a, yPredict = b, xTrue_NUll = an, yPredict_NUll = bn, limit = 50 ) 
...
valid_x3, _, _ = Data_Aggregate_Clip_Scal(["M10WD","M8WD","M9WD","M10WE","M8WE","M9WE"], 
                                                     Clipper=Clipper_train_x3, Scaler=Scaler_train_x3) # _代表不需要用到
...
model.summary()
modelcallbacks = model.fit(x = train_x3, y = train_y1,
                epochs=5, batch_size=64, shuffle=True,
                validation_data=(valid_x3, valid_y1,))
#Save & Load weights
...
model.compile(loss = my_mean_squared_error, optimizer =  adam, metrics=[my_mean_absolute_error]) #mean_absolute_error
model.summary()
modelcallbacks = model.fit(x = [train_x1,train_x2,train_x3], y = train_y1,
                epochs=10, batch_size=64, shuffle=True,
                validation_data=([valid_x1,valid_x2,valid_x3], valid_y1,))
...
model.compile(loss = my_mean_squared_error, optimizer =  adam, metrics=[my_mean_absolute_error]) #mean_absolute_error
model.summary()
modelcallbacks = model.fit(x = [train_x1,train_x2,train_x3,train_x1_cul,train_x2_cul,train_x3_cul], y = train_y1,
                epochs=5, batch_size=64, shuffle=True,
                validation_data=([valid_x1,valid_x2,valid_x3,valid_x1_cul,valid_x2_cul,valid_x3_cul], valid_y1,))
...
plt.show()
# In[]: 2. Performance_table
#  生成預測值
train_y1_01 = np.where(train_y1> 0, 1, 0)
train_y1_hat_NUll = train_x3/3 #null modle
#train_y1_hat = model.predict(train_x3) #M1
train_y1_hat = model.predict([train_x1,train_x2,train_x3]) #M2
#train_y1_hat = model.predict([train_x1,train_x2,train_x3,train_x1_cul,train_x2_cul,train_x3_cul]) #M3
train_y1_hat_01 = np.where(train_y1_hat> 0, 1, 0)
#np.savetxt("train_y1_hat.csv", train_y1_hat, delimiter=",") #輸出CSV

valid_y1_01 = np.where(valid_y1> 0, 1, 0)
valid_y1_hat_NUll = valid_x3/3 #null modle
#valid_y1_hat = model.predict(valid_x3) #M1
valid_y1_hat = model.predict([valid_x1,valid_x2,valid_x3]) #M2
#valid_y1_hat = model.predict([valid_x1,valid_x2,valid_x3,valid_x1_cul,valid_x2_cul,valid_x3_cul]) #M3
valid_y1_hat_01 = np.where(valid_y1_hat> 0, 1, 0)
...
for i in range( 0,(train_y1.shape)[0]): #每一列(會員)，用實際的Item vs 預測的Item，來計算一個confusion matrix
    cm = confusion_matrix(y_true = train_y1_01[i,:],
                     y_pred = train_y1_hat_01[i,:] )
...
#M1_performance_table_NUll = pd.DataFrame(data=cm_temp)
#M1_performance_table_NUll.rename(columns={0:'True1', 1:'Predict1_of_true1',2:'Predict1',3:'true1_of_Predict1'},inplace=True)#注意这里0和1都不是字符串
# valid
cm_temp = [] #先創建一個LIST for Valid Set
for i in range( 0,(train_y1.shape)[0]): #每一列(會員)，用實際的Item vs 預測的Item，來計算一個confusion matrix
    cm = confusion_matrix(y_true = valid_y1_01[i,:],
                     y_pred = valid_y1_hat_01[i,:])
    if cm.shape == (2, 2): #防止某些會員Item購買量全為0，若發生此狀況，則該會員的performance不記錄
        cm_temp.append([(cm[1,0] +  cm[1,1]) , cm[1,1], (cm[0,1] +  cm[1,1]) , cm[1,1]])
    if i % 1000 == 0 :  print(i)
...
M1_performance_table_valid.rename(columns={0:'True1', 1:'Predict1_of_true1',
                2:'Predict1',3:'true1_of_Predict1'},inplace=True)#注意这里0和1都不是字符串
#M1_performance_table_valid_NUll = pd.DataFrame(data=cm_temp)
#M1_performance_table_valid_NUll.rename(columns={0:'True1', 1:'Predict1_of_true1', 2:'Predict1',3:'true1_of_Predict1'},inplace=True)#注意这里0和1都不是字符串     

#Train extra predict
a = np.where( train_y1_01 - train_y1_hat_NUll > 0, 1, 0)
b = np.where( (train_y1_hat_01 > 0) & (a > 0) , 1, 0)
cm_temp = [] #先創建一個LIST
for i in range( 0,(train_y1.shape)[0]): #每一列(會員)，用實際的Item vs 預測的Item，來計算一個confusion matrix
    a_temp = sum(a[i,:])
    b_temp = sum(b[i,:] )
    cm_temp.append([a_temp, b_temp])
    if i % 1000 == 0 :
        print(i)
M1_performance_table_extrapredict = pd.DataFrame(data=cm_temp)
M1_performance_table_extrapredict.rename(columns={0:'Number_not_in_NullModel', 1:'Number_not_in_NullModel_but_predict'},inplace=True)#注意这里0和1都不是字符串

#Valid extra predict
a = np.where( valid_y1_01 - valid_y1_hat_NUll > 0, 1, 0)
b = np.where( (valid_y1_hat_01 > 0) & (a > 0) , 1, 0)
cm_temp = [] #先創建一個LIST
for i in range( 0,(train_y1.shape)[0]): #每一列(會員)，用實際的Item vs 預測的Item，來計算一個confusion matrix
    a_temp = sum(a[i,:])
    b_temp = sum(b[i,:] )
    ...
    if i % 1000 == 0 :
        print(i)
M1_performance_table_valid_extrapredict = pd.DataFrame(data=cm_temp)
M1_performance_table_valid_extrapredict.rename(columns={0:'Number_not_in_NullModel', 1:'Number_not_in_NullModel_but_predict'},inplace=True)#注意这里0和1都不是字符串
...
MyScatterplot( xTrue = a, yPredict = b, xTrue_NUll = an, yPredict_NUll = bn, limit = 100 ) 
#MyScatterplot( xTrue = a, yPredict = b, limit = 100 ) 
MyScatterplot( xTrue = c, yPredict = d, xTrue_NUll = cn, yPredict_NUll = dn, limit = 100) 
...
a = M1_performance_table_extrapredict['Number_not_in_NullModel']
b = M1_performance_table_extrapredict['Number_not_in_NullModel_but_predict']
MyScatterplot( xTrue = a, yPredict = b, xTrue_NUll = a, yPredict_NUll = b, limit = 200 ) 
print('Valid Extra Predict')
a = M1_performance_table_valid_extrapredict['Number_not_in_NullModel']
b = M1_performance_table_valid_extrapredict['Number_not_in_NullModel_but_predict']
MyScatterplot( xTrue = a, yPredict = b, xTrue_NUll = a, yPredict_NUll = b, limit = 200 ) 
    # In[]: 4. 直方圖
r=b/a #recall
r=r.loc[~r.isna()]
