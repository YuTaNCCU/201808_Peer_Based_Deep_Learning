...
def Generate_Item_cross_entropy(y_true, y_pred):
    ...
    for i in range( 0,(y_true.shape)[1]): 
        ...
        if i % 100 == 0 :
            print(i)
    performance_table = pd.DataFrame(data=temp)
    performance_table.rename(columns={0:'corrcoef_temp'},inplace=True)#注意这里0和1都不是字符串
    return performance_table

M3_Item_corrcoef = Generate_Item_cross_entropy(y_true=train_y3_inverse_transform, y_pred=train_y3_hat_M2_inverse_transform)
M3_Item_corrcoef.head()

cross_entropy0 = cross_entropy(predictions=train_y3_01, targets=train_y3_hat_NUll_01)
cross_entropy1 = cross_entropy(predictions=train_y3_01, targets=train_y3_hat_M1_01)
cross_entropy2 = cross_entropy(predictions=train_y3_01, targets=train_y3_hat_M2_01)
cross_entropy3 = cross_entropy(predictions=train_y3_01, targets=train_y3_hat_M3_01)
ys=(cross_entropy0,cross_entropy1,cross_entropy2,cross_entropy3)
labels=('0','1','2','3')
...
model2.summary()
modelcallbacks = model2.fit(x = [train_x1_cul,train_x2_cul,train_x3_cul], y = train_y3,
                epochs=5, batch_size=64, shuffle=True,
