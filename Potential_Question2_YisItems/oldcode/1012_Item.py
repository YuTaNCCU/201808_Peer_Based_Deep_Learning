...
SelectedMb = pd.read_csv('data/SelectedMb.csv', encoding = 'big5hkscs' ,squeeze=True,header =None)  #return a Series
...
# In[]: call tokenize_AllItem function
...
# In[]: 建立 所有的FirstNumber, SecondNumber
All_FirstandSecondNumber =  OriginData_Processed.loc[OriginData_Processed.資料日期 <= "2017-10-31",["單品名稱","品號-品名稱","群號-群名稱"]]
All_FirstandSecondNumber = All_FirstandSecondNumber.groupby("單品名稱").first()
All_FirstandSecondNumber["FirstNumber"] = All_FirstandSecondNumber["品號-品名稱"].astype('category').cat.codes #給定品號一編號
All_FirstandSecondNumber["SecondNumber"] = All_FirstandSecondNumber["群號-群名稱"].astype('category').cat.codes #給定群號一編號
All_FirstandSecondNumber.reset_index(level="單品名稱", drop=False, inplace= True)
All_FirstandSecondNumber.to_csv("data/All_FirstandSecondNumber.csv",index=False)#寫入檔案

#All_FirstandSecondNumber["FirstNumber"].unique().shape # 98種 :   -1 -->1001  +1002 +MbID =100
#All_FirstandSecondNumber["SecondNumber"].unique().shape # 423種 :   -1 -->1001  +1002 +MbID =425

# In[]: tokenize_MbRecord_FirstandSecondNumber() 所有會員購買的的 SecondNumber to token > 再轉換為寬表格
def tokenize_MbRecord_FirstandSecondThirdNumber(Qdata_Item, All_group):
#Qdata_Item = data_temp[:2000]
#All_group = All_FirstandSecondNumber[['單品名稱','FirstNumber']]
    """
    >將包含購買資料的大資料表，轉換為以「MbID」為單位的寬表格(稀疏表格)
    """
    ...
    #------------------------------------------------------------------------------------------------#
    # texts_to_sequences : 將Q_Item的 "單品名稱"欄位 轉換成 "item"欄位
    All_group_temp = All_group.set_index('單品名稱') #將單品名稱設定成index，以利搜尋
    List_temp= []#創建一個List
    #單品名稱或SecondNumber轉換成ItemToken
    for j in range(len( Qdata_Item )):  
        str_temp = Qdata_Item.loc[j, "單品名稱" ]
        if str_temp == "空值" :
            List_temp.append( 10001 ) #空值
        elif All_group_temp.index.contains( str_temp ) :
            List_temp.append(  All_group_temp.loc[ str_temp ][0])
        else :
            List_temp.append( 10002 ) #未見過的群號
        if j%10000 == 0:  #顯示進度 #未見過的群號
            clear_output()
            print( j, "/" , len( Qdata_Item ) , datetime.datetime.now() )
    ...
    # 未使用到的單品名稱，仍要有欄位
    # .1找出沒使用到的ItemToken
    All_group_unique = All_group.iloc[:,1].unique()
    #FutureWarning
    #All_group_unique  = All_group_unique[All_group_unique!="空值"] #空值已納入101 #FutureWarning
    All_group_unique = np.append( All_group_unique, [10001, 10002]) # 1001=空值, 1002=未見過的群號
    NoShowItem = All_group_unique [ np.isin(All_group_unique,Qdata_Item['ItemToken'], invert=True) ] #All_group_unique未在Qdata_Item['ItemToken']者，要生出其欄位
    NoShowItem = NoShowItem[NoShowItem!=-1] #-1=空值,空值已納入10
    NoShowItem=NoShowItem.astype(str)
    
    # .2創建包含沒使用到的ItemToken作為欄位的空np陣列
    df_temp = pd.DataFrame(
        data = np.zeros(NoShowItem.shape[0]*Qdata_Item_sparse.shape[0]).reshape(Qdata_Item_sparse.shape[0],NoShowItem.shape[0]),
                      columns=NoShowItem)
    Qdata_Item_sparse2 = pd.concat([Qdata_Item_sparse, df_temp], axis =1 ) #合併DataFrame(左)與np陣列(右)
    # .3排序欄位名稱append
    Qdata_Item_sparse2 =  Qdata_Item_sparse2.drop(['MbID'], axis=1).copy() #先去掉欄位MbID
    #Qdata_Item_sparse2.columns = [int(i) for i in Qdata_Item_sparse2.columns] #欄位轉成整數
    Qdata_Item_sparse2.sort_index(axis=1, inplace=True) #排序欄位名稱
    Qdata_Item_sparse3 = pd.concat([Qdata_Item_sparse['MbID'].astype(int) ,Qdata_Item_sparse2], axis=1) #加回欄位MbID
    #------------------------------------------------------------------------------------------------#
    # Customers who didn't show in the quarter
    NoShowMb = SelectedMb[  ~ SelectedMb.isin(Qdata_Item_sparse3.loc[:,'MbID'])].reset_index(drop=True) # 該季未出現的會員Dataframe
    df_temp = pd.DataFrame(
        data = np.zeros(NoShowMb.shape[0] * (Qdata_Item_sparse3.shape[1])).reshape(NoShowMb.shape[0], (Qdata_Item_sparse3.shape[1])),
                      columns=Qdata_Item_sparse3.columns)
    df_temp.MbID = NoShowMb
    
    Qdata_Item_sparse3 = pd.concat([Qdata_Item_sparse3, df_temp], 
                                  axis = 0,sort=False).sort_values(by=["MbID"]).reset_index(drop=True) #合併成一表
    # NaN補0
    Qdata_Item_sparse3.fillna(value = 0, inplace=True)
    #------------------------------------------------------------------------------------------------#
    return (Qdata_Item_sparse3)

# In[]: 呼叫tokenize_MbRecord_FirstandSecondThirdNumber，生成各個月份的 Mb Item Sparse Table
MbItemSparseTableFirstNumber = {} #創建一個DICTIONARY
MbItemSparseTableSecondNumber = {} #創建一個DICTIONARY
MbItemSparseTableThirdNumber = {} #創建一個DICTIONARY
for month in range(1,13):
    for weekday in ["WD","WE"]:#"WD","WE"
        ...
        str_temp = "M" + str(month) + str(weekday) #創建DICTIONARY 的KEY
        """print("#-----------------------------------------------",'FirstNumber', str_temp,"-----------------------------------------#")
        #First Number
        MbItemSparseTableFirstNumber[str_temp]  = tokenize_MbRecord_FirstandSecondThirdNumber(Qdata_Item = data_temp, 
                                       All_group = All_FirstandSecondNumber[['單品名稱','FirstNumber']]) #呼叫tokenize_MbRecord，並把回傳的DATAFRAME存在DICTIONARY
        MbItemSparseTableFirstNumber[str_temp].to_csv('FirstNumber'+str_temp + ".csv") #個別存成csv
        print("#-----------------------------------------------",'SecondNumber', str_temp,"-----------------------------------------#")
        #Second Number     
        MbItemSparseTableSecondNumber[str_temp]  = tokenize_MbRecord_FirstandSecondThirdNumber(Qdata_Item = data_temp, 
                                       All_group = All_FirstandSecondNumber[['單品名稱','SecondNumber']]) #呼叫tokenize_MbRecord，並把回傳的DATAFRAME存在DICTIONARY
        MbItemSparseTableSecondNumber[str_temp].to_csv('SecondNumber'+str_temp + ".csv") #個別存成csv
        """
        print("#-----------------------------------------------",'單品名稱', str_temp,"-----------------------------------------#")
        #Third Number   
        MbItemSparseTableThirdNumber[str_temp] = tokenize_MbRecord_FirstandSecondThirdNumber(Qdata_Item = data_temp, 
                                       All_group = All_Item2000.loc[1:2000,['單品名稱','ItemToken']]) #呼叫tokenize_MbRecord，並把回傳的DATAFRAME存在DICTIONARY
        #1:2000是因為原本0給未見過的商品，但後來設計:1001=空值, 1002=未見過的商品
        MbItemSparseTableThirdNumber[str_temp].to_csv('ThirdNumber'+str_temp + ".csv") #個別存成csv
# In[]: Load Data
#MbItemSparseTable = {} 
Item1 = {} 
Item2 = {} 
...
for month in range(1,3):
   for weekday in ["WD","WE"]:
       str_temp = "M" + str(month) + str(weekday) #創建DICTIONARY 的KEY    
       Item1[str_temp] = pd.read_csv('FirstNumber'+str_temp + ".csv") #讀取First Number
       Item2[str_temp] = pd.read_csv('SecondNumber'+str_temp + ".csv") #讀取Second Number
       Item2[str_temp] = pd.read_csv('ThirdNumber'+str_temp + ".csv") #Third Number
...
for month in range(1,13):
    for weekday in ["WD","WE"]:
        ...
        str_temp = "M" + str(month) + str(weekday) #創建DICTIONARY 的KEY
        print("#-----------------------------------------------",'FirstNumber', str_temp,"-----------------------------------------#")
        MbItemSparseTableFirstNumber[str_temp].columns.shape
        print("#-----------------------------------------------",'SecondNumber', str_temp,"-----------------------------------------#")
        #Second Number     
        MbItemSparseTableSecondNumber[str_temp].columns.shape
