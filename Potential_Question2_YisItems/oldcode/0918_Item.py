...
All_FirstandSecondNumber =  OriginData_Processed.loc[OriginData_Processed.資料日期 <= "2017-10-31",["單品名稱","FirstNumber","SecondNumber"]]
...
# In[]: tokenize_MbRecord_FirstNumber 所有會員購買的的FirstNumber to token > 再轉換為寬表格
#def tokenize_MbRecord_FirstNumber(Qdata_Item, All_FirstandSecondNumber):
Qdata_Item = OriginData_Processed.iloc[:100,:]
All_group = All_FirstandSecondNumber[['單品名稱','SecondNumber']]
"""
...
All_group.iloc[:,1].unique().shape
All_group_unique  = All_group_unique[All_group_unique!="空值"] #空值已納入101
All_group_unique = np.append( All_group_unique, [1001, 1002]) # 101=空值, 102=未見過的群號
...
NoShowItem=NoShowItem.astype(int)
...
# In[]: tokenize_MbRecord_SecondNumber() 所有會員購買的的 SecondNumber to token > 再轉換為寬表格
#def tokenize_MbRecord_SecondNumber(Qdata_Item, All_FirstandSecondNumber):
Qdata_Item = OriginData_Processed.iloc[:100,:]
All_group = All_FirstandSecondNumber[['單品名稱','SecondNumber']]
"""
>將包含購買資料的大資料表，轉換為以「MbID」為單位的寬表格(稀疏表格)
...
for j in range(len( Qdata_Item )):  
    str_temp = Qdata_Item.loc[j, "單品名稱" ]
    if str_temp == "空值":
        List_temp.append( '101' ) #空值
    elif All_group_temp.index.contains( str_temp ) :
        List_temp.append(  All_group_temp.loc[ str_temp ][0])
    else :
        List_temp.append( '102' ) #未見過的群號
    if j%10000 == 0:  #顯示進度 #未見過的群號
        clear_output()
        print( j, "/" , len( Qdata_Item ) , datetime.datetime.now() )
...
# 未使用到的單品名稱，仍要有欄位
# .1找出沒使用到的ItemToken
All_group_unique = All_group.loc[:,"FirstNumber"].unique()
All_group_unique  = All_group_unique[All_group_unique!="空值"] #空值已納入101
All_group_unique = np.append( All_group_unique, [101, 102]) # 101=空值, 102=未見過的群號
NoShowItem = All_group_unique [ np.isin(All_group_unique,Qdata_Item['ItemToken'], invert=True) ] #All_group_unique未在Qdata_Item['ItemToken']者，要生出其欄位
NoShowItem=NoShowItem.astype(int)
