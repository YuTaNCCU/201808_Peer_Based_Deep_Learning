...
def Scatterplot_RP( performance_table, NUll_performance_table, lower = 0, upper = 500) : 
    ...
    plt.scatter(x=xTrue_NUll, y=yPredict_NUll, marker =  ".", color='blue', alpha =0.1) 
    
    plt.scatter(x=xTrue, y=yPredict, marker =  ".", color='red', alpha =0.1) 
    ...
    #網格線
    plt.grid()
    plt.savefig( 'Scatterplot_RP_Recall' + '_' + str(upper)+ str(lower) + '_' + str(upper) + '.png', dpi = 300)
    plt.show()
    
    print('Precision')
    xTrue = performance_table['Predict1']
    yPredict= performance_table['true1_of_Predict1']
    ...
    #網格線
    plt.grid()
    plt.savefig( 'Scatterplot_RP_Precision' + str(lower) + '_' + str(upper) + '.png', dpi = 300)
    plt.show()   
  #In[]:
...
#看哪一個模型好
df_temp = pd.DataFrame(data={'0':Null_temp,'1':M1_temp,'2':M2_temp,'3':M3_temp})
df_temp.fillna(0, inplace = True)
df_temp['BestModel'] = df_temp.idxmax(axis=1)
df_temp.BestModel.fillna('fare', inplace = True) #Na當作平手
for i in range(0, df_temp.shape[0]): #分數一樣的當作平手
...
train_y3, Clipper_train_y3, Scaler_train_y3 = Data_Aggregate_Clip_Scal(["M10WD","M10WE"], 
                                                     Dataset = Item3 ,Clipper=None, Scaler=None)
train_y3_inverse_transform = Scaler_train_y3.inverse_transform(train_y3)
...
valid_y3_inverse_transform = Scaler_valid_y3.inverse_transform(valid_y3)
...
NUll_performance_table = generate_performance_table_RP(y_true=train_y3_01, y_pred=train_y3_hat_NUll_01)
NUll_performance_table_valid = generate_performance_table_RP(y_true=valid_y3_01, y_pred=valid_y3_hat_NUll_01)
...
model1.compile(loss = my_mean_squared_error, optimizer =  adam, metrics=[my_mean_absolute_error]) #mean_absolute_error
model1.summary()
modelcallbacks = model1.fit(x = train_x3, y = train_y3,
                epochs=5, batch_size=64, shuffle=True,
                validation_data=(valid_x3, valid_y3,))
#Save & Load weights
model1.save(  'M1_model_weights_'+ str(datetime.datetime.now() ).replace(" ", "-").replace(":", "_").replace(".", "_")+'.h5'  )
#model1=load_model('save_M1_model_weights_2018-09-20-08_57_45_810670.h5',custom_objects={'my_mean_squared_error': my_mean_squared_error, 'my_mean_absolute_error':my_mean_absolute_error})
# In[]: 2.M1生出預測值
train_y3_hat_M1 = model1.predict(train_x3) 
train_y3_hat_M1_inverse_transform = Scaler_train_y3.inverse_transform(train_y3_hat_M1)
train_y3_hat_M1_01 = np.where(train_y3_hat_M1_inverse_transform> 0, 1, 0)
#np.savetxt("train_y3_hat.csv", train_y3_hat, delimiter=",") #輸出CSV

valid_y3_hat_M1 = model1.predict(valid_x3) 
valid_y3_hat_M1_inverse_transform = Scaler_train_y3.inverse_transform(valid_y3_hat_M1)
valid_y3_hat_M1_01 = np.where(valid_y3_hat_M1_inverse_transform> 0, 1, 0)
...
M1_performance_table = generate_performance_table_RP(y_true=train_y3_01, y_pred=train_y3_hat_M1_01)
M1_performance_table_valid = generate_performance_table_RP(y_true=valid_y3_01, y_pred=valid_y3_hat_M1_01)
...
M1_performance_table_CompareWithNull = Generate_Performance_table_CwN(train_y3_01, train_y3_hat_NUll, train_y3_hat_M1_01)
M1_performance_table_valid_CompareWithNull = Generate_Performance_table_CwN(valid_y3_01, valid_y3_hat_NUll, valid_y3_hat_M1_01)
...
model2.summary()
modelcallbacks = model2.fit(x = [train_x1,train_x2,train_x3], y = train_y3,
                epochs=5, batch_size=64, shuffle=True,
...
train_y3_hat_M2_inverse_transform = Scaler_train_y3.inverse_transform(train_y3_hat_M2)
train_y3_hat_M2_01 = np.where(train_y3_hat_M2_inverse_transform> 0, 1, 0)

valid_y3_hat_M2 = model2.predict([valid_x1,valid_x2,valid_x3]) #M2
valid_y3_hat_M2_inverse_transform = Scaler_train_y3.inverse_transform(valid_y3_hat_M2)
valid_y3_hat_M2_01 = np.where(valid_y3_hat_M2_inverse_transform> 0, 1, 0)
# In[] 3. M2畫散布圖 Scatterplot_RP + Compare With NullModel
#呼叫函數generate_performance_table_RP，來生成performance_table
M2_performance_table = generate_performance_table_RP(y_true=train_y3_01, y_pred=train_y3_hat_M2_01)
M2_performance_table_valid = generate_performance_table_RP(y_true=valid_y3_01, y_pred=valid_y3_hat_M2_01)
...
#呼叫函數Generate_Performance_table_CwN，來生成 Performance_table_CwN
M2_performance_table_CompareWithNull = Generate_Performance_table_CwN(train_y3_01, train_y3_hat_NUll, train_y3_hat_M2_01)
M2_performance_table_valid_CompareWithNull = Generate_Performance_table_CwN(valid_y3_01, valid_y3_hat_NUll, valid_y3_hat_M2_01)
...
y = Dense(2404, activation='relu',name='Output')(P)
# In[]:M3-3 只使用累積資料
A1 = Input(shape = (99,),name='Input_1')#不往後連接
A2 = Input(shape = (424,),name='Input_2')#不往後連接
A3 = Input(shape = (2404,),name='Input_3')#不往後連接
A1_cul = Input(shape = (99,),name='Input_1_cul') 
A2_cul = Input(shape = (424,),name='Input_2_cul') 
A3_cul = Input(shape = (2404,),name='Input_3_cul') 
...
for i in range(1,12):   
    ...
    model3.summary()
    modelcallbacks = model3.fit(x = [train_x1,train_x2,train_x3,train_x1_cul,train_x2_cul,train_x3_cul], y = train_y3,
    ...
    #model3=load_model('M3_model_weights_2018-10-04-08_47_38_235990.h5',custom_objects={'my_mean_squared_error': my_mean_squared_error, 'my_mean_absolute_error':my_mean_absolute_error})
# In[]:M3-4 把M2的資料換成累積
A1 = Input(shape = (99,),name='Input_1')
A2 = Input(shape = (424,),name='Input_2')
A3 = Input(shape = (2404,),name='Input_3')
...
model3.compile(loss = my_mean_squared_error, optimizer =  adam, metrics=[my_mean_absolute_error]) #mean_absolute_error
model3.summary()
modelcallbacks = model3.fit(x = [train_x1_cul,train_x2_cul,train_x3_cul], y = train_y3,
                epochs=5, batch_size=64, shuffle=True,
                validation_data=([valid_x1_cul,valid_x2_cul,valid_x3_cul], valid_y3,))
...
train_y3_hat_M3 = model3.predict([train_x1,train_x2,train_x3,train_x1_cul,train_x2_cul,train_x3_cul]) #M3
train_y3_hat_M3_inverse_transform = Scaler_train_y3.inverse_transform(train_y3_hat_M3)
train_y3_hat_M3_01 = np.where(train_y3_hat_M3_inverse_transform> 0, 1, 0)
#np.savetxt("train_y3_hat.csv", train_y3_hat, delimiter=",") #輸出CSV

valid_y3_hat_M3 = model3.predict([valid_x1,valid_x2,valid_x3,valid_x1_cul,valid_x2_cul,valid_x3_cul]) #M3
valid_y3_hat_M3_inverse_transform = Scaler_train_y3.inverse_transform(valid_y3_hat_M3)
valid_y3_hat_M3_01 = np.where(valid_y3_hat_M3_inverse_transform> 0, 1, 0)
#np.savetxt("valid_y3_hat.csv", valid_y3_hat, delimiter=",") #輸出CSV

# In[] 3. M3畫散布圖 Scatterplot_RP + Compare With NullModel
#呼叫函數generate_performance_table_RP，來生成performance_table
M3_performance_table = generate_performance_table_RP(y_true=train_y3_01, y_pred=train_y3_hat_M3_01)
M3_performance_table_valid = generate_performance_table_RP(y_true=valid_y3_01, y_pred=valid_y3_hat_M3_01)
...
#呼叫函數generate_performance_table_RP，來生成performance_table
M3_performance_table_CompareWithNull = Generate_Performance_table_CwN(train_y3_01, train_y3_hat_NUll, train_y3_hat_M3_01)
M3_performance_table_valid_CompareWithNull = Generate_Performance_table_CwN(valid_y3_01, valid_y3_hat_NUll, valid_y3_hat_M3_01)
