...
# 選擇至少出現過2季
SelectedMb = temp3.loc[ temp3.Na<=2 , "會員卡號"]#至少出現過2季：57159人,3季:44249人,4季:31893人
...
All_Item8000 = tokenize_AllItem(Qdata = OriginData_Processed.loc[OriginData_Processed.資料日期 <= "2017-10-31",:],
                                max_features = 8000) #8295種
All_Item8000.to_csv("data/All_Item8000.csv",index=False)#寫入檔案
#All_Item = pd.read_csv('data/All_Item8000.csv' )


# In[]: 所有會員購買的單品名稱 to token > 再轉換為寬表格
def tokenize_MbRecord(FilteredData, All_Item_maxfeatures):
    """
...
for month in range(3,13):
    for weekday in ["WD","WE"]:
        ...
        MbItemSparseTable[str_temp]  = tokenize_MbRecord(FilteredData = data_temp,
                         All_Item_maxfeatures = All_Item2000 ) #呼叫tokenize_MbRecord，並把回傳的DATAFRAME存在DICTIONARY
