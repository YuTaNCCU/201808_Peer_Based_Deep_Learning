...
# In[]: 2.M1生出預測值
train_y3_hat = model.predict(train_x3) 
...
valid_y3_hat = model.predict(valid_x3) 
...
#呼叫函數generate_performance_table_RP，來生成performance_table
M2_performance_table = generate_performance_table_RP(y_true=train_y3_01, y_pred=train_y3_hat_01)
M2_performance_table_valid = generate_performance_table_RP(y_true=valid_y3_01, y_pred=valid_y3_hat_01)
...
#呼叫函數generate_performance_table_RP，來生成performance_table
M2_performance_table_CompareWithNull = Generate_Performance_table_CwN(train_y3_01, train_y3_hat_NUll, train_y3_hat_01)
M2_performance_table_valid_CompareWithNull = Generate_Performance_table_CwN(valid_y3_01, valid_y3_hat_NUll, valid_y3_hat_01)
...
A3_cul = Input(shape = (2404,),name='Input_3_cul') #不往後連接 
Merged = concatenate([A1, A2, A3])
B = Dense(1000,activation='relu')(Merged)
B = Dropout(0.4)(B)
C = Dense(500,activation='relu',)(B)
...
for i in range(1,2):   
    ...
    if modelcallbacks.history['val_my_mean_absolute_error'][0] !=1.3977330695430434 :
        model.save(  'M3_model_weights_'+ str(datetime.datetime.now() ).replace(" ", "-").replace(":", "_").replace(".", "_")+'.h5'  )
    #model=load_model('M3_model_weights_2018-10-03-15_23_34_191206.h5',custom_objects={'my_mean_squared_error': my_mean_squared_error, 'my_mean_absolute_error':my_mean_absolute_error})
...
def generate_performance_table_RP(y_true, y_pred):
    ...
    for i in range( 0,(train_y3.shape)[1]): 
...
#呼叫函數generate_performance_table_RP，來生成performance_table
M3_performance_table = generate_performance_table_RP(y_true=train_y3_01, y_pred=train_y3_hat_01)
M3_performance_table_valid = generate_performance_table_RP(y_true=valid_y3_01, y_pred=valid_y3_hat_01)
...
#呼叫函數generate_performance_table_RP，來生成performance_table
M3_performance_table_CompareWithNull = Generate_Performance_table_CwN(train_y3_01, train_y3_hat_NUll, train_y3_hat_01)
M3_performance_table_valid_CompareWithNull = Generate_Performance_table_CwN(valid_y3_01, valid_y3_hat_NUll, valid_y3_hat_01)
