...
P = Dropout(0.4)(P)
y = Dense(2001, activation='relu',name='Output')(P)
...
for i in range(0,1) :
    ...
    model1.compile(loss = my_mean_squared_error, optimizer =  adam, metrics=[fbeta_score, my_mean_squared_error, my_mean_absolute_error]) #mean_absolute_error
    model1.summary()
    modelcallbacks = model1.fit(x = train_x3, y = train_x3,
                    epochs=10, batch_size=64, shuffle=True,
                    validation_data=(train_x3, train_y3_True,))
    #Save & Load weights
...
for i in range(0,1) :
    ...
    Now = datetime.datetime.now()
    model1.save(  'M1_model_weights_'+ str( Now ).replace(" ", "-").replace(":", "_").replace(".", "_")+'.h5'  )
    #model1=load_model('M1_model_weights_2018-10-16-08_13_02_890869.h5',custom_objects={'fbeta_score':fbeta_score, 'fbeta_bce_loss':fbeta_bce_loss,'my_mean_squared_error': my_mean_squared_error, 'my_mean_absolute_error':my_mean_absolute_error})
    #SummarizeHistory(modelcallbacks)
...
# In[]將模型f時間、權重、modelcallbacks儲存到檔案
with open("your_file.txt", "w") as f:
    for s in History_temp:
        f.write(str(s) +"\n")
score=[]
...
train_y3_hat_01 = np.where(train_y3_hat_inverse_transform> 0.1, 1, 0)
...
model2.compile(loss = my_mean_squared_error, optimizer =  adam, metrics=[my_mean_squared_error, my_mean_absolute_error]) #mean_absolute_error
model2.summary()
modelcallbacks = model2.fit(x = [train_x1,train_x2,train_x3], y = train_y3_True_01,
