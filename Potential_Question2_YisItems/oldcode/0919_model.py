...
for month in range(1,13):
   for weekday in ["WD","WE"]:
       ...
       Item1[str_temp] = pd.read_csv('FirstNumber'+str_temp + ".csv").drop(columns=["Unnamed: 0","MbID"]).values  #讀取品號First Number
       Item2[str_temp] = pd.read_csv('SecondNumber'+str_temp + ".csv").drop(columns=["Unnamed: 0","MbID"]).values  #讀取群號Second Number
...
adagrad = optimizers.Adagrad(lr=0.00002, epsilon=None, decay=0.0)
...
model.compile(loss = my_mean_squared_error, optimizer =  adagrad, metrics=[my_mean_absolute_error]) #mean_absolute_error
model.summary()
modelcallbacks = model.fit(x = train_x1, y = train_y1,
                epochs=10, batch_size=64, shuffle=True,
                validation_data=(valid_x1, valid_y1,))
#Save & Load weights
model.save(  'model_weights_'+ str(datetime.datetime.now() ).replace(" ", "-").replace(":", "_").replace(".", "_")+'.h5'  )

#model=load_model('model_weights_recall0.43_高pre0.41_2018-09-12-16_51_41_024437.h5',custom_objects={'my_mean_squared_error': my_mean_squared_error, 'my_mean_absolute_error':my_mean_absolute_error})
